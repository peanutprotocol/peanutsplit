...
test('formatting excludes hash-bound generated bytes while the real-artifact validator stays unskippable', () => {
	const ignored = fs
	...
	// The bug this pins: the validator once ran only when CI exported an artifact root, so eight
	// of fifteen guides compiled to an HTTP 500 with `pnpm test` green. It reads the committed
	// artifact itself now, and nothing may put a switch back in front of it.
	assert.match(validator, /SPLIT_CONTENT_GENERATED_ROOT/)
	assert.doesNotMatch(validator, /process\.env\./)
	assert.doesNotMatch(validator, /\.runIf\(|\.skipIf\(|\.skip\(|it\.skip\b|describe\.skip\b/)

	const ciWorkflow = fs.readFileSync(path.join(REPO_ROOT, '.github/workflows/ci.yml'), 'utf8')
	assert.doesNotMatch(ciWorkflow, /SPLIT_CONTENT_ARTIFACT_ROOT/)
	assert.match(ciWorkflow, /^ {6}- run: pnpm test$/m)
})
