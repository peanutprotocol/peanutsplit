...
## Standing items (updated 2026-09-01)
- **Record now lives in-repo**: edit `repo/ops/steward/*.md`; cron `peanutsplit-steward-sync`
  (*/10) pushes them to main. ⚠️ Its recovery check is unsound — it clears the failure
  sentinel and posts a green when there is nothing left to STAGE, even while commits sit
  unpushed (observed 2026-09-01). Verify with `git status -sb` / the GitHub file size, not
  the Discord green. Fix proposed in INCIDENTS 2026-09-01 00:45Z; box tooling, Hugo's call.
- **Detector noise**: `SVC-DOWN … replicas=2/1` is a rolling deploy, not an outage. Alert
  should trigger on running<desired only. Proposed, not applied.
- **Open user report** (2026-08-23): loading-screen flicker on installed PWA / Android
  Chrome. Needs design judgement — not PR-able under the zero-regression bar.
- **Still BLIND**: no Sentry read token for the Peanut org (`/root/.split-sentry-read-token`).

## Session crons (armed 2026-09-01, session-only — re-arm after any session restart)
- `13,43 * * * *` shallow sweep: edge 200, services 1/1, bus delta, latest deploy status,
  new FeedbackReport rows, steward-sync FAIL/[ahead] check, Sentry if token present.
  Clean = one line in chat only; findings = INCIDENTS.md + Discord one-liner.
- `17 9 * * *` daily deep sweep: shallow + volumes vs trailing baseline, 24h logs grouped,
  backup/R2 verification, disk/RAM, Kuma down-events, box-cron count (expect 5). Always
  writes a dated INCIDENTS.md entry (one line if clean).
- ⚠️ Session crons auto-expire after 7 days and die with the session — the tmux watchdog's
  recovery prompt re-arms them; verify with CronList after any restart.

## Posture delta (Hugo, 2026-09-01)
Fix lane widened one notch: for a CLEARLY-obvious, zero-regression fix the supervisor may
open a PR via split-gh AND merge it once gates pass. Never push main directly. Anything
less than obvious: flag only (INCIDENTS.md + Discord), never fix.
