 ...
 */
async function proxyDispatcher(tag: string): Promise<unknown | null> {
    ...
    try {
        ...
        console.error(`[${tag}] proxy unavailable`, err instanceof Error ? err.name : 'unknown')
        return null
    }
}
...
async function callGemini(call: ModelCall, config: ModelConfig): Promise<string> {
    const dispatcher = await proxyDispatcher(call.tag)
