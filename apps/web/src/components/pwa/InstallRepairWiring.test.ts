...
const appPage = (): string => readFileSync(new URL('../../app/(product-shell)/app/page.tsx', import.meta.url), 'utf8')
...
describe('legacy room-shortcut repair wiring', () => {
    ...
    it('keeps the automatic repair notice conditional, independently dismissible, and actionable', () => {
        ...
        expect(prompt).toContain("visible ? t(repairExposure ? 'repair.announcement' : 'announcement') : ''")
    })

    it('keeps a healthy installation status-only and a suspected room shortcut actionable', () => {
        const row = component('InstallRow')

        expect(row).toContain("if (state === 'installed')")
        expect(row).toContain('<StateRow label={label} line={t(\'row.installed\')} testId="install-row-installed" />')
        expect(row).toContain("if (state === 'repair')")
