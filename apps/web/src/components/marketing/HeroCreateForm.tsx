 ...
 * readable part is exact; the tail is a 128-bit capability minted server-side, so it shows as
 * `SLUG_TAIL_HINT` until the room exists. Inventing a tail here would be a promise the next
 * screen breaks — and the dots have to carry the tail's SHAPE, or the preview teaches a URL the
 * next screen does not deliver.
 *
 * The catalog is the bundled one rather than `useCurrencies`, which arrives stale by design and
 * would put a network round-trip on every view of a page whose whole job is the first paint. The
 * only thing the live read adds is a `hasRate` correction, and this form does not price anything.
 *
...
export function HeroCreateForm({
    ...
    useEffect(() => {
        ...
        if (next && FALLBACK_CURRENCIES.some((info) => info.code === next)) setCurrency(next)
    }, [hints, currencyChosen])
     ...
     *  the page already does. The draw-your-own editor is exempt: it portals outside this
     *  subtree, so dismissing on its strokes would unmount the sheet being drawn in. */
    const pickerRef = useRef<HTMLDetailsElement>(null)
    useEffect(() => {
        if (!pickerOpen || drawingOpen) return
        const onPointerDown = (event: PointerEvent) => {
            if (!pickerRef.current?.contains(event.target as Node)) setPickerOpen(false)
        }
        document.addEventListener('pointerdown', onPointerDown)
        return () => document.removeEventListener('pointerdown', onPointerDown)
    }, [pickerOpen, drawingOpen])
