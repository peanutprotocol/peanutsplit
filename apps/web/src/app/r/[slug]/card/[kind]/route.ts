...
import { enforceRateLimit, LOOKUP_MISS_LIMIT } from '@/server/rateLimit'
...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string; kind: string }> }) {
    ...
    const [fonts, card] = await Promise.all([ogFonts(), loadAchievementCard(slug, kind, alterEgo).catch(() => null)])
    ...
    return new ImageResponse(renderAchievementCard(card), {
        ...OG_SIZE,
        fonts,
