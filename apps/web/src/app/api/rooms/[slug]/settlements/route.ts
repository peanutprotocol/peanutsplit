...
export const POST = (request: Request, ctx: Ctx) =>
    respond(async () => {
        ...
        if (!isMember(body.fromId)) throw badRequest('payer is not a member of this room')
        if (!isMember(body.toId)) throw badRequest('payee is not a member of this room')
        if (body.fromId === body.toId) throw badRequest('a settlement needs two different people')
