...
async function verifyLegacyOrigin(legacyBaseArgument) {
    ...
    const manifestResponse = await fetch(new URL('/manifest.webmanifest', legacy), { redirect: 'manual' })
    invariant(
    ...
    const documentResponse = await fetch(new URL('/app', legacy), { redirect: 'manual' })
